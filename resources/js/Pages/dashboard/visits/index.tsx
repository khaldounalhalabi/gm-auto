import ActionsButtons from "@/Components/datatable/ActionsButtons";
import DataTable from "@/Components/datatable/DataTable";
import Visit from "@/Models/Visit";
import Http from "@/Modules/Http/Http";
import { Link } from "@inertiajs/react";

const Index = ({ exportables }: { exportables: string[] }) => {
    return (
        <DataTable
            title="Visit Table"
            createUrl={route("v1.web.protected.visits.create")}
            importRoute={route("v1.web.protected.visits.import")}
            exportRoute={route("v1.web.protected.visits.export")}
            importExampleRoute={route("v1.web.protected.visits.import.example")}
            exportables={exportables}
            getDataArray={(res) => res.data}
            getTotalPages={(res) => res?.paginate?.total_pages ?? 0}
            getTotalRecords={(res) => res.paginate?.total ?? 0}
            api={(
                page?: number | undefined,
                search?: string | undefined,
                sortCol?: string | undefined,
                sortDir?: string | undefined,
                perPage?: number | undefined,
                params?: object | undefined,
            ) =>
                Http.make<Visit[]>().get(
                    route("v1.web.protected.visits.data"),
                    {
                        page: page,
                        search: search,
                        sort_col: sortCol,
                        sort_dir: sortDir,
                        limit: perPage,
                        ...params,
                    },
                )
            }
            schema={[
                {
                    name: "id",
                    label: "ID",
                    sortable: true,
                },
                { name: "date", label: "Date", sortable: true },
                {
                    name: "car.model_name",
                    label: "Car Model",
                    render: (cell, record, setHidden, revalidate) => {
                        return (
                            record?.car_id && (
                                <Link
                                    className="hover:text-primary underline"
                                    href={route(
                                        "v1.web.protected.cars.show",
                                        record?.car_id,
                                    )}
                                >
                                    {`${record?.car?.model_name} - ${record?.car?.registration_plate}`}
                                </Link>
                            )
                        );
                    },
                },
                {
                    name: "client.full_name",
                    label: "Client Full Name",
                    render: (cell, record, setHidden, revalidate) => {
                        return (
                            record?.client_id && (
                                <Link
                                    className="hover:text-primary underline"
                                    href={route(
                                        "v1.web.protected.clients.show",
                                        record?.client_id,
                                    )}
                                >
                                    {record?.client?.full_name}
                                </Link>
                            )
                        );
                    },
                },
                {
                    name: "cost",
                    label: "Cost",
                    sortable: true,
                },
                {
                    name: "total_cost",
                    label: "Total Cost",
                    sortable: true,
                },
                {
                    label: "Options",
                    render: (_data, record, setHidden, revalidate) => (
                        <ActionsButtons
                            buttons={["delete", "edit", "show"]}
                            baseUrl={route("v1.web.protected.visits.index")}
                            id={record?.id ?? 0}
                            setHidden={setHidden}
                        />
                    ),
                },
            ]}
        />
    );
};

export default Index;
